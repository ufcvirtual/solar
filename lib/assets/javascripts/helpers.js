...
function flash_message(msg, css_class, div_to_show) {
  ...
  var flash_message_placeholder = "flash_message_placeholder";
  $(html).hide().appendTo(div_to_show).fadeIn("medium", function() {
    $("#flash_message")
      .effect("highlight","medium")
      .closest(".sticky-wrapper").addClass("flash_message_placeholder");
  });
}

function erase_flash_messages() {
  if ($('#flash_message')) {
    $('#flash_message').closest(".sticky-wrapper").removeClass("flash_message_placeholder");
    $('#flash_message').fadeOut("medium",function(){
      $(this).remove();
    })
  }
}
