...
class  CommentFileTest < ActiveSupport::TestCase
	...
	test "arquivo deve ter nome" do
  	comment_file = CommentFile.create(:comment_id => assignment_comments(:ac2).id)
  ...
  test "arquivo valido" do
    comment_file = CommentFile.create(comment_id: assignment_comments(:ac2).id, attachment: fixture_file_upload('files/assignments/comment_files/teste1.txt', 'text/plain'))
