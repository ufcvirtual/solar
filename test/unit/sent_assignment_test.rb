...
class  SentAssignmentTest < ActiveSupport::TestCase
  ...
  fixtures :assignments, :sent_assignments

  test "nota deve ser maior ou igual a 0" do
    sent_assignment = SentAssignment.create(:assignment_id => assignments(:a4).id, :user_id => users(:aluno1).id, :grade => -2)
    ...
    sent_assignment = SentAssignment.create(:assignment_id => assignments(:a4).id, :user_id => users(:aluno1).id, :grade => 12)
  ...
  test "se tiver grupo, user_id deve ser nulo" do
    sent_assignment = SentAssignment.create(:assignment_id => assignments(:a4).id, :user_id => users(:aluno1).id, :group_assignment_id => group_assignments(:a1).id)

    assert sent_assignment.user_id.nil?
  end

  test "send assignment valido" do
    sent_assignment = SentAssignment.create(:assignment_id => assignments(:a4).id, :user_id => users(:aluno1).id, :grade => 7)
