class AssignmentFilesController < ApplicationController
  ...
  def create
    ...
    group, @assignment = sent_assignment.group_assignment, sent_assignment.assignment
    @own_assignment = ( sent_assignment.user_id == current_user.id or (not(group.nil?) and group.user_in_group?(current_user.id)) )
