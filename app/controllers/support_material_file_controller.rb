class SupportMaterialFileController < ApplicationController
  ...
  def download_all_file_ziped
    ...
    file_names = SupportMaterialFile.search_files(allocation_tag_id).collect{|file| [file["attachment_file_name"]]}
    ...
    zip_in_test = "tmp/"+Digest::SHA1.hexdigest(file_names.to_s)+".zip"
    ...
    existing_zip_files.each do |file_test|
    ...
    if !exist_zip
      ...
      Zip::ZipFile.open("tmp/#{Digest::SHA1.hexdigest(file_names.to_s)}.zip", Zip::ZipFile::CREATE) { |zipfile|
        file_names.each do |zipers|
    ...
    zip_name = Zip::ZipFile.open(Digest::SHA1.hexdigest(file_names.to_s)+".zip", Zip::ZipFile::CREATE).to_s
  ...
  def download_folder_file_ziped
    ...
    file_names = SupportMaterialFile.search_files(allocation_tag_id, folder).collect{|file| [file["attachment_file_name"]]}
