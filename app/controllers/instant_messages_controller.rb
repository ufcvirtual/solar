...
class InstantMessagesController < ApplicationController
  ...
  def prebind
      # @client = Jabber::HTTPBinding::Client.new("#{current_user.username}"+ @_dominio +"/im")
      # puts "#{current_user.username}"+ @dominio +"/im"
      # @client.connect('http://'+ @_ip +':' + @_porta + '/http-bind/')
      ...
      #         :bosh_service_url => 'http://'+ @_ip +':' + @_porta + '/http-bind/' }
