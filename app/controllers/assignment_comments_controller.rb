class AssignmentCommentsController < ApplicationController
  ...
  def index
    @assignment = AcademicAllocationUser.find_one(params[:academic_allocation_user_id]).assignment
    render partial: 'list', locals: { comments: academic_allocation_user.assignment_comments }
  end
  ...
  def new
    ...
    @assignment_comment = AssignmentComment.new academic_allocation_user_id: academic_allocation_user.id
  ...
  def download
    ...
    raise CanCan::AccessDenied unless Assignment.owned_by_user?(current_user.id, { academic_allocation_user: file.assignment_comment.academic_allocation_user }) || is_observer_or_responsible
  ...
  private
    ...
    def assignment_comment_params
      params.require(:assignment_comment).permit(:academic_allocation_user_id, :comment, files_attributes: [:id, :attachment, :_destroy])
    end
