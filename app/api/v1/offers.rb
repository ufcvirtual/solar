      ...
      namespace :offer do 
        ...
        post "/" do
          begin
            ...
            offer_end = (params[:ma] ? (params[:offer_end].try(:to_date) + 6.month) : params[:offer_end].try(:to_date))
            offer = creates_offer_and_semester(params[:name], {start_date: params[:offer_start].try(:to_date), end_date: offer_end}, {start_date: params[:enrollment_start], end_date: params[:enrollment_end]}, {curriculum_unit_id: curriculum_unit_id, course_id: course_id})
            {id: offer.id}
          end
        end
