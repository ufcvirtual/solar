  ...
  def show_breadcrumb
    ...
    [breadcrumb].flatten.each_with_index do |bread, idx|
      unless bread.nil?
        ...
        text_bread << "&nbsp; #{I18n.t('mysolar.closed_uc')}" if idx == [breadcrumb].flatten.size - 1 && !AllocationTag.includes(:offers).find(active_tab[:url][:allocation_tag_id]).verify_offer_period rescue ''
      end
    end
