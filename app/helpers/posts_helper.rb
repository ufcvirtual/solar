...
HTML
  ...
  def attachments(post, editable = false, can_interact = false)
    ...
    files.each do |file|
      ...
      html_files <<     "<div class='audio' id='audio-#{file.id}' data-type='#{file.attachment_file_name.split(".").last}' data-source='#{download_post_post_file_url(post, file)}'></div>" if file.attachment_content_type.index('audio') or file.attachment_file_name.last(4).eql?('.aac')
      html_files << '</li>'
    end
