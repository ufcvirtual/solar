class Exam < Event
  ...
  def self.correction_cron
    # query = "auto_correction = TRUE AND ((schedules.end_date < current_date AND (result_release IS NULL OR result_release <= NOW())) OR ((schedules.start_date < current_date OR (schedules.start_date = current_date AND start_hour::time < current_time)) AND exams.immediate_result_release=TRUE))"
    query = "auto_correction = TRUE AND ((schedules.end_date < current_date AND (result_release IS NULL OR result_release <= NOW()))"
