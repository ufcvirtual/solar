  ...
  def self.generate_text_to_audio(text, path, language="pt-BR")
    begin
      client = Google::Cloud::TextToSpeech.text_to_speech do |config|
        config.credentials = "/home/cristiano/dev/ruby/solar/config/marine-outpost-380517-bf6a85c5fb96.json"
      end
    rescue Exception => error
      p error
      errors.add(:count_text, 'Google Authorization failed')
    end
    begin
      client = Google::Cloud::TextToSpeech.text_to_speech
    rescue Exception => error
      p error
      errors.add(:count_text, 'Google client failed')
    end
    input = { text: text }
    voice = {
    ...
    begin
      ...
      errors.add(:count_text, 'Google failed: Erro ao sintetizar audio')      # Handle exceptions that subclass Google::Cloud::Error
    end
