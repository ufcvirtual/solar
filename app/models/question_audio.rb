class QuestionAudio < ActiveRecord::Base
  ...
  def self.list(question_id)
    QuestionAudio.where(question_id: question_id)
      .select('DISTINCT question_audios.id, question_audios.audio_file_name, question_audios.audio_content_type, question_audios.audio_file_size, question_audios.audio_updated_at')
  end
end
