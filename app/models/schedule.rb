class Schedule < ActiveRecord::Base
  ...
  def self.events(allocation_tags, period = false, date_search = nil)
    ...
    unless allocation_tags.nil? 
      ...
      lessons_events = Lesson.joins(:schedule, {lesson_module: :academic_allocations}).where(where_date).select("'lesson' AS schedule_type, lessons.name AS name, lessons.description, schedules.start_date, schedules.end_date")
    end
