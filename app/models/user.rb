class User < ActiveRecord::Base
  ...
  def allocation_tags_ids_with_access_on(actions, controller, all=false, include_nil=false, no_groups=false, no_offers=false)
    ...
    allocations     = Allocation.joins(profile: :resources).where("resources.action IN (?)", actions.flatten).where(resources: { controller: controller }, allocations: { status: Allocation_Activated, user_id: id })
