class User < ActiveRecord::Base
  ...
  def allocation_tags_ids_with_access_on(actions, controller, all=false, include_nil=false)
    ...
    allocations     = Allocation.joins(profile: :resources).where("resources.action IN (?)", actions).where(resources: { controller: controller }, allocations: { status: Allocation_Activated, user_id: id }).select('DISTINCT allocation_tag_id, allocations.id')
