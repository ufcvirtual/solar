  ...
  private
    ...
    def have_permission_access?(user, action, object_class, object)
      ...
      if object.respond_to?(:allocation_tag) || (object_class != User && object.respond_to?(:allocation_tags))
        ...
        at_of_user   = user.allocation_tags.joins(:allocations).where(allocations: { profile_id: profiles.map(&:id), status: Allocation_Activated.to_i }).to_a.collect!{|at| at.related(all_or_lower)}.flatten.uniq ## allocations do usuario com perfil para executar a acao
