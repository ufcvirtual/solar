      ...
      tools.each do |tool|
        ...
        when 'exams'
          "(SELECT DISTINCT
            ...
            case
            when (current_date < s.start_date) OR (current_date = s.start_date AND ((exams.start_hour IS NOT NULL AND exams.end_hour != '' AND current_time<to_timestamp(exams.start_hour, 'HH24:MI:SS')::time))) then 'not_started'
            when exams.immediate_result_release=TRUE AND academic_allocation_users.grade IS NULL AND ((last_attempt.complete=TRUE OR (exams.uninterrupted=TRUE AND SUM(exam_responses.duration)>0)) AND (exams.attempts = user_attempts.count)) then 'finished'
            ...
            when (last_attempt.complete=TRUE OR (exams.uninterrupted=TRUE AND SUM(exam_responses.duration)>0)) AND (exams.attempts = user_attempts.count) then 'finished'
            when ((current_date >= s.start_date AND (exams.start_hour IS NULL OR exams.start_hour = '' OR current_time>= to_timestamp(exams.start_hour, 'HH24:MI:SS')::time) AND current_date <= s.end_date AND (exams.end_hour IS NULL OR exams.end_hour = '' OR current_time<=to_timestamp(exams.end_hour, 'HH24:MI:SS')::time))) AND (exams.uninterrupted=TRUE AND SUM(exam_responses.duration)=0) AND (exams.attempts = user_attempts.count) then 'to_answer'
            else
              'not_answered'
            end AS situation,
            academic_allocations.evaluative,
